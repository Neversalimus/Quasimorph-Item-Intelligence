...
Write-Host 'Item Intelligence v1.7.42.5-test5 - MCM Chinese font fix + Large UI + Implant sources + Languages' -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { throw "DEV build/stage failed with exit code $LASTEXITCODE" }

$stageDll = Join-Path $stage 'ItemIntelligence.dll'
$stageManifest = Join-Path $stage 'modmanifest.json'
if (-not (Test-Path -LiteralPath $stageDll -PathType Leaf)) { throw 'DEV stage validation failed: ItemIntelligence.dll missing.' }
if (-not (Test-Path -LiteralPath $stageManifest -PathType Leaf)) { throw 'DEV stage validation failed: modmanifest.json missing.' }
$stageHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $stageDll).Hash

Write-Host ''
Write-Host 'DEV BUILD + STAGING OK.' -ForegroundColor Green
...
Write-Host '[ItemIntelligence] ACTIVE VERSION 1.7.42.5-test5 (McmFontLanguagesTest05).' -ForegroundColor Cyan
Write-Host ''
